...
public class PublicBlogRestControllerTest extends AbstractRestControllerTest {
    ...
    public void getPublicBlogPosts_ShouldReturnAllPublicBlogPosts() throws Exception {
        ...
        mockMvc.perform(get(API_ROOT + URL_SITE_BLOGS))
    ...
    public void getPublicBlogPostById_ShouldReturnBlogPost() throws Exception {
        ...
        mockMvc.perform(get(API_ROOT + URL_SITE_BLOGS_BLOG, BLOG_ID))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaTypes.HAL_JSON))
                ...
                .andExpect(jsonPath("$._links.comments.href", endsWith(URL_SITE_BLOGS+"/"+BLOG_ID+"/comments{?page,size,sort}")))
    ...
    public void getBlogApprovedCommentPosts_ShouldReturnApprovedComments() throws Exception {
        ...
        when(blogPostRepositoryMock.findOne(BLOG_ID)).thenReturn(blog);
        when(commentPostRepositoryMock.findByBlogPostAndStatusOrderByCreatedTimeAsc(
                any(BlogPost.class), eq(CommentStatusType.APPROVED), any(Pageable.class)))
            .thenReturn(page);
        
        mockMvc.perform(get(API_ROOT + URL_SITE_BLOGS_BLOG_COMMENTS, BLOG_ID))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaTypes.HAL_JSON))
    ...
    public void getBlogApprovedCommentPostById_ShouldReturnApprovedComment() throws Exception {
        ...
        mockMvc.perform(get(API_ROOT + URL_SITE_BLOGS_BLOG_COMMENTS_COMMENT, BLOG_ID, COMMENT_ID))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaTypes.HAL_JSON))
                ...
                .andExpect(jsonPath("$._links.self.href", endsWith(COMMENT_ID.toString())))
                .andExpect(jsonPath("$._links.blog.href", endsWith(URL_SITE_BLOGS+"/"+BLOG_ID)))
                .andExpect(jsonPath("$._links.author.href", endsWith(URL_SITE_PROFILES+"/"+comment.getAuthor().getUserId())))
                ;
