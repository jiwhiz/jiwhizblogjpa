...
public class AuthorBlogCommentResourceAssembler implements ResourceAssembler<CommentPost, AuthorBlogCommentResource> {
    ...
    public AuthorBlogCommentResource toResource(CommentPost comment) {
        ...
        resource.add(linkTo(methodOn(AuthorBlogCommentRestController.class).getCommentPostById(comment.getBlogPost().getId(), comment.getId()))
                .withSelfRel());
        resource.add(linkTo(methodOn(AuthorBlogRestController.class).getAuthorBlogPostById(comment.getBlogPost().getId()))
                .withRel(AuthorBlogCommentResource.LINK_NAME_BLOG));
        resource.add(linkTo(methodOn(WebsiteRestController.class).getUserProfile(comment.getAuthor().getUserId()))
